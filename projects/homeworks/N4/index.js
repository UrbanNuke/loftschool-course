...
const collectDOMStat = root => {
    const initial = {tags: {}, classes: {}, texts: 0};

    const recursiveFn = element => {
        [...element.childNodes].forEach(child => {
            ...
            if (child.nodeType === 1) {
                initial.tags[child.tagName] = initial.tags[child.tagName] ? initial.tags[child.tagName] + 1 : 1;
                child.classList?.forEach(cls => {
                    initial.classes[cls] = initial.classes[cls] ? initial.classes[cls] + 1 : 1;
                });
            }
        });
    };
...
const observeChildNodes = (where, fn) => {
    const observer = new MutationObserver(changes => {
        changes.forEach(change => {
            fn({
                type: change.removedNodes?.length ? 'remove' : 'insert',
                nodes: change.removedNodes?.length ? [...change.removedNodes] : [...change.addedNodes]
            });
        });
    });
